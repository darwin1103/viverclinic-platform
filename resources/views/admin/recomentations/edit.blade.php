...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.recomentations.update', $recommendation->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="description" name="description" class="form-control" rows="2" required maxlength="500">{{ old('description', $recommendation->description) }}</textarea>
                        </div>
                        <div class="mb-3">
                            <label for="image" class="form-label">Imagen (Opcional)</label>
                            @if($recommendation->image)
                                <div class="mb-2">
                                    <img src="{{ Storage::url($recommendation->image) }}" class="img-thumbnail" style="max-height: 150px;">
                                </div>
                            @endif
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="content" name="content" class="form-control ckeditor-textarea">{{ old('content', $recommendation->content) }}</textarea>
                        </div>
                        <button type="submit" class="btn btn-primary">Actualizar</button>
                        <a href="{{ route('admin.recomentations.index') }}" class="btn btn-secondary">Cancelar</a>
                    </form>
                </div>
            </div>
        </div>
